...
export class PreciosPersonalAlmacenHistService {
  ...
  cargarPrecios(desde:number=0) {
    ...
    const url = `${ base_url }/precio/personalAlmacenHist?desde=${desde}`;
    return this.http.get<CargarPrecioPersonalAlamacen>( url, this.headers );        
  }


  crearPrecio(formData:PrecioPersonalAlamacenForm){
    return this.http.post(`${base_url}/precio/personalAlmacenHist`,formData)
  }

  actualizarPrecios(uid:string,precio :PrecioPersonalAlamacen) {
    const url = `${ base_url }/precio/personalAlmacenHist/${uid}`;
