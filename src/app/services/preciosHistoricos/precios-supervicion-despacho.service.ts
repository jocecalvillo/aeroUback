...
export class PreciosSupervicionDespachoHistService {
  ...
  cargarPrecios(desde:number=0) {
    ...
    const url = `${ base_url }/precio/supervicionDespachoHist?desde=${desde}`;
    return this.http.get<CargarPrecioSupervicionDespacho>( url, this.headers );        
  }

  crearPrecio(formData:PrecioSupervicionDespachoForm){
    return this.http.post(`${base_url}/precio/supervicionDespachoHist`,formData)
  }

  actualizarPrecios(uid:string,precio :PrecioSupervicionDespacho) {
    const url = `${ base_url }/precio/supervicionDespachoHist/${uid}`;
