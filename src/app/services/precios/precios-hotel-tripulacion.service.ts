...
export class PreciosHotelTripulacionService {
  ...
  cargarPrecios(desde:number=0) {
    ...
    const url = `${ base_url }/precio/hotelTripulacion?desde=${desde}`;
  ...
  crearPrecio(formData:PrecioHotelTripulacionForm){
    return this.http.post(`${base_url}/precio/hotelTripulacion`,formData)
  }

  actualizarPrecios(uid:string,precio :PrecioHotelTripulacion) {
    const url = `${ base_url }/precio/hotelTripulacion/${uid}`;
