...
export class PreciosSupervicionDespachoCamService {
  ...
  cargarPrecios(desde:number=0) {
    ...
    const url = `${ base_url }/precio/supervicionDespachoCam?desde=${desde}`;
  ...
  crearPrecio(formData:PrecioSupervicionDespachoCamForm){
    return this.http.post(`${base_url}/precio/supervicionDespachoCam`,formData)
  }

  actualizarPrecios(uid:string,precio :PrecioSupervicionDespachoCam) {
    const url = `${ base_url }/precio/supervicionDespachoCam/${uid}`;
